...
try:
    ...
    # --- НАЧАЛО: применение рамок и цветной заливки таблицы ---

    sheet_id = sheet._properties['sheetId']  # Получаем id листа

    def apply_borders_and_colors(spreadsheet_id, start_row, num_rows, num_cols, sheet_id):
        requests = []

        # Границы вокруг всего диапазона и внутри
        requests.append({
            "updateBorders": {
                "range": {
                    "sheetId": sheet_id,
    ...
    apply_borders_and_colors(
        "1SjT740pFA7zuZMgBYf5aT0IQCC-cv6pMsQpEXYgQSmU",
        start_row,
        len(values),
        len(values[0]),
        sheet_id
    )
    # --- КОНЕЦ: применение рамок и цветной заливки таблицы ---
